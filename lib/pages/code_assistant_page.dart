...
import 'package:google_generative_ai/google_generative_ai.dart' hide FunctionResponse; // Avoid naming conflict
...
class ChatMessage {
  ...
  Content toContent() {
     ...
     if (type == MessageType.text) {
         ...
         final imageText = text.isNotEmpty ? text : "[User sent an image: ${imagePath?.split('/').last}]";
         // For now, represent image as text in history
         return isUserMessage ? Content.text(imageText) : Content.model([TextPart(text)]);
     } else {
         String historyText = text;
         if (type == MessageType.audio) historyText = "[User sent audio recording. Transcription: \"$text\"]";
...
class _CodeAssistantPageState extends State<CodeAssistantPage> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _longTermMemoryService = Provider.of<LongTermMemoryService>(context, listen: false);
    ...
    _chatHistoryService = Provider.of<ChatHistoryService>(context, listen: false);
    ...
    _playerStateSubscription = _audioPlayer.onPlayerStateChanged.listen((state) {
        ...
        if (state == PlayerState.stopped || state == PlayerState.completed){
            setState(() {
                ...
                if(state == PlayerState.completed) _currentlyPlayingAudioPath = null; // Deselect on completion
            });
        }
    });
  ...
  Future<void> _loadInitialChat() async {
    setState(() { _isInitializing = true; _isLoading = true; });
    ...
    if (initialChatId != null) {
     ...
     if (mounted){
         setState(() { _isInitializing = false; _isLoading = false; });
     }
  }
  ...
  Future<void> _loadChat(String sessionId) async {
     ...
     if (session != null && mounted) {
        ...
        await _initializeChat(history: _getChatHistoryBuffer()); // Initialize Gemini with loaded history
     } else {
  ...
  Future<void> _initializeChat({List<Content>? history}) async {
    if (!mounted) return;
    setState(() { _isInitializing = true; _isLoading = true; _model = null; _chat = null; });
...
final systemInstruction = _settingsService.systemInstruction;

    final ltmSaveDeclaration = FunctionDeclaration(
          "save_long_term_memory", // Consistent naming convention
          ...
          Schema(
            SchemaType.object,
            properties: {
              'key': Schema(SchemaType.string, description: 'A short, descriptive key or topic for the memory item (e.g., "user_preference_color", "project_deadline").'),
              'content': Schema(SchemaType.string, description: 'The actual information or data to be saved.'),
            },
         ...
         final ltmRetrieveDeclaration = FunctionDeclaration(
          "retrieve_long_term_memory", // Consistent naming
           "Retrieves saved information from the assistant's long-term memory based on a query or topic.",
           Schema(
             SchemaType.object,
             properties: {
               'query': Schema(SchemaType.string, description: 'The key, topic, or search term to look for in the long-term memory (e.g., "user_preference_color", "project", "deadline").'),
             },
...
final customDeclarations = _customToolService.functionDeclarations;
final allToolDeclarations = [ltmSaveDeclaration, ltmRetrieveDeclaration, ...customDeclarations];
final toolsList = allToolDeclarations.isNotEmpty
    ? [Tool(functionDeclarations: allToolDeclarations)]
    : null;

    try {
      _model = GenerativeModel(
           model: 'gemini-2.0-flash-lite', // Or 'gemini-1.5-flash' if available and supports images/tools
           ...
           toolConfig: toolsList != null ? ToolConfig(
               functionCallingConfig: FunctionCallingConfig(mode: FunctionCallingMode.auto)
           ) : null,
       );

      // Start chat with provided history (could be empty or from loaded chat)
      // Use getChatHistoryBuffer to enforce buffer size limit from settings IF starting fresh
      final effectiveHistory = history ?? _getChatHistoryBuffer();
      _chat = _model!.startChat(history: effectiveHistory);

      if (mounted) {
        setState(() { _isInitializing = false; _isLoading = false; });
        debugPrint("Chat initialized. History length: ${effectiveHistory.length}. Tools: ${allToolDeclarations.map((e) => e.name).join(', ')}");
      }
        ...
        if (mounted) {
            setState(() { _isInitializing = false; _isLoading = false; });
            _addMessage("Error initializing AI Model: $e", isUser: false, type: MessageType.error);
        }
    }
  }
  ...
  List<Content> _getChatHistoryBuffer() {
    ...
    return _messages
        ...
        .where((content) => content.role != 'error') // Filter out invalid content placeholders
        .toList()
        .reversed
        .toList();
  }
  ...
  void _addMessage(
    ...
    if (!isUser && _messages.isNotEmpty && _messages.last.text == text && (_messages.last.type == MessageType.error || _messages.last.type == MessageType.functionResponse)) {
 ...
 void _scrollToBottom({int durationMillis = 300, bool jump = false}) {
     ...
     showModalBottomSheet(
        ...
        builder: (ctx) => Padding(
            ...
            child: Column(
              ...
              children: [
                  Text("Send Image?", style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 10),
                  Image.file(imageFile, height: 150, fit: BoxFit.contain),
                  const SizedBox(height: 10),
                  TextField(
                    controller: _textController, // Reuse main controller temporarily
                    decoration: const InputDecoration(hintText: 'Add optional text...'),
                    maxLines: 3,
                   ),
  ...
  Future<void> _startRecording() async {
     ...
     if (!success && mounted) {
         ...
         ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Failed to start recording. Check permissions?"), backgroundColor: Colors.red)
         );
     }
  }
  ...
  Future<void> _stopRecording({bool cancelled = false}) async {
    ...
    if (cancelled) {
        ...
        if(result.path != null) File(result.path!).delete().catchError((e) => print("Error deleting cancelled recording: $e"));
        return;
    }
    ...
    if (result.success && result.path != null) {
      ...
      setState(() { _isTranscribing = true; }); // Show transcribing indicator
    ...
    //  final audioFile = result.file!;
      ...
      // Get audio duration for display before transcription
       Duration? duration;
       try {
           final tempPlayer = AudioPlayer(); // Use a temporary player to get duration
      ...
      // Then, transcribe
       ...
       setState(() { _isTranscribing = false; }); // Hide transcribing indicator

       if (transcription != null && !transcription.startsWith("Error:") && !transcription.startsWith("Exception:") ) {
            ...
            // Send the transcription text to Gemini
             ...
             // int messageIndex = _messages.lastIndexWhere((m) => m.audioPath == audioPath);
             // if (messageIndex != -1) {
             //    setState(() { _messages[messageIndex] = // recreate message with text });
             // }
       } else {
  ...
  Future<void> _playPauseAudio(String path) async {
     if (_currentlyPlayingAudioPath == path && _currentPlayerState == PlayerState.playing) {
         // Pause
         await _audioPlayer.pause();
      } else {
         // Stop previous playback if different file
         if (_currentlyPlayingAudioPath != null && _currentlyPlayingAudioPath != path) {
         ...
         // Play or Resume
         setState(() { _currentlyPlayingAudioPath = path; });
          try {
              await _audioPlayer.play(DeviceFileSource(path)); // Use DeviceFileSource for local files
          } catch (e) {
              print("Error playing audio: $e");
              ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text("Error playing audio: $e"), backgroundColor: Colors.red)
              );
              setState(() { _currentlyPlayingAudioPath = null; }); // Reset on error
          }
      }
  }
  ...
  Future<void> _sendMessageWithData({File? imageFile, String? transcription}) async {
    ...
    bool isUserAction = true; // Assume user action unless only transcription is sent without UI interaction
    ...
    if (imageFile != null) {
       ...
       try {
           Uint8List imageBytes = await imageFile.readAsBytes();
            ...
            if (imageFile.path.toLowerCase().endsWith('.webp')) mimeType = 'image/webp';
            if (imageFile.path.toLowerCase().endsWith('.heic')) mimeType = 'image/heic';
            if (imageFile.path.toLowerCase().endsWith('.heif')) mimeType = 'image/heif';
           ...
           print("Error reading image file: $e");
            _addMessage("Error processing image file.", isUser: false, type: MessageType.error);
      ...
      isUserAction = false; // This is result of transcription, not direct typing
    ...
    if (isUserAction) { // Only clear text input if it was a direct user send action
        _textController.clear();
    }
    ...
    try {
        final content = Content.multi(parts); // Use multiPart for image/text combo
        ...
        } else if ((responseText == null || responseText.trim().isEmpty) && functionCalls.isEmpty ) {
             print("Received empty response without function calls.");
              _addMessage("Received an empty response.", isUser: false, type: MessageType.error);
         }

    } on GenerativeAIException catch (e) {
        print("GenerativeAIException sending message: ${e.message}");
         _addMessage("API Error: ${e.message}.", isUser: false, type: MessageType.error);
    } catch (e) {
        print("Error sending message: $e");
         _addMessage("Sorry, an error occurred: $e", isUser: false, type: MessageType.error);
    } finally {
        if (mounted) {
            setState(() { _isLoading = false; });
        }
  ...
  Future<void> _handleFunctionCall(FunctionCall call) async {
      ...
      String functionResponseText;
      dynamic resultData;
      MessageType responseType = MessageType.functionResponse;
        ...
        switch (call.name) {
          ...
          // ... LTM Save logic ...
            ...
            if (key != null && content != null) {
              functionResponseText = await _longTermMemoryService.saveMemoryItem(key, content);
               resultData = {'status': 'Success', 'message': functionResponseText};
            } else {
               functionResponseText = "Error: Missing 'key' or 'content' argument.";
               resultData = {'status': 'Error', 'message': functionResponseText};
               responseType = MessageType.error; // Indicate error type
            }
            break;
          case 'retrieve_long_term_memory':
           // ... LTM Retrieve logic ...
            final query = call.args['query'] as String?;
            if (query != null) {
              functionResponseText = _longTermMemoryService.retrieveMemoryItems(query);
              resultData = {'status': 'Success', 'retrieved_data': functionResponseText};
            } else {
               functionResponseText = "Error: Missing 'query' argument.";
               resultData = {'status': 'Error', 'message': functionResponseText};
          ...
          default:
            if (_customToolService.customTools.any((t) => t.name == call.name)) {
               // ... Custom Tool logic ...
                resultData = await _customToolService.executeCustomTool(call.name, call.args);
                functionResponseText = "Executed custom tool '${call.name}'. Result: ${jsonEncode(resultData)}";
            } else {
               functionResponseText = "Error: Unknown function requested: ${call.name}";
               resultData = {'status': 'Error', 'message': functionResponseText};
               responseType = MessageType.error;
            }
        }

        // Add UI message indicating function result (optional but helpful)
         _addMessage(
              "Function (${call.name}): ${functionResponseText.split(':').last.trim()}", // Show concise result/status
              isUser: false,
              type: responseType, // Use determined type
          );


        // --- Send Function Response back to the Model ---
         final responseContent = Content.functionResponse(call.name, resultData);
         final responseAfterFunc = await _chat?.sendMessage(responseContent);

        // Add model's response *after* the function call
         final responseText = responseAfterFunc?.text;
         if (responseText != null && responseText.trim().isNotEmpty) {
             _addMessage(responseText, isUser: false, type: MessageType.text);
         } else if (responseAfterFunc?.functionCalls.isNotEmpty ?? false) {
             for (final nextCall in responseAfterFunc!.functionCalls) {
                 await _handleFunctionCall(nextCall);
             }
         } else {
         ...
         _addMessage("Error processing function call ${call.name}: $e", isUser: false, type: MessageType.error);
      }
  }
    ...
    return Consumer<ChatHistoryService>(
      builder: (context, historyService, child) {
        return Scaffold(
          ...
          appBar: AppBar(
            title: Text(_currentChatSession?.displayTitle ?? 'Gemini Assistant'),
             leading: IconButton( // Button to open drawer
            ...
            actions: [
              IconButton(
                ...
                onPressed: () {
                   ...
                   _audioPlayer.pause();
                   setState(() { _currentlyPlayingAudioPath = null; }); // Deselect audio

                    Navigator.push(
                       context,
                       MaterialPageRoute(builder: (context) => const SettingsPage()),
                   ).then((_) {
                       // Optional: Re-initialize if critical settings changed
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      if (!_isRecording) // Show spinner only if not recording
      ...
      child: Column(
        children: [
          AppBar(
             ...
             automaticallyImplyLeading: false, // Remove back button in drawer header
             actions: [
                IconButton(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                 ...
                 return ListTile(
                    ...
                    subtitle: Text('Last activity: ${session.lastModified.toLocal().toString().substring(0,16)}',
                      ...
                      onPressed: () async {
                          final confirm = await showDialog<bool>(
                              ...
                              builder: (ctx) => AlertDialog(
                                ...
                                content: Text('Are you sure you want to delete the chat "${session.displayTitle}"? This cannot be undone.'),
                                actions: [
                          ...
                          if (confirm == true) {
                              ...
                              // If the deleted chat was active, start new
                              if (isActive) {
                                  _startNewChat(); // This will trigger UI update
                              }
                             // No need to manually pop drawer unless deletion takes long
                          }
                      },
                    ),
                    onTap: () {
 ...
 final margin = message.isUserMessage
 ? const EdgeInsets.only(top: 4, bottom: 4, left: 60, right: 8)
 : const EdgeInsets.only(top: 4, bottom: 4, right: 60, left: 8);

 Color bubbleColor;
 Color textColor = colorScheme.onSurface; // Default text based on surface

     switch (message.type) {
 case MessageType.error:
 bubbleColor = colorScheme.errorContainer; // Use theme error container
 textColor = colorScheme.onErrorContainer;
 break;
 case MessageType.functionResponse:
 bubbleColor = colorScheme.tertiaryContainer; // Example using tertiary
 textColor = colorScheme.onTertiaryContainer;
 break;
 case MessageType.audio:
 // Use slightly different variants for user/ai
// Example icon colors
      Color sliderActiveColor = message.isUserMessage ? colorScheme.secondary : colorScheme.primary;
      // ignore: deprecated_member_use
      Color _ = sliderActiveColor.withOpacity(0.3);


 bubbleColor = message.isUserMessage ? colorScheme.secondaryContainer : colorScheme.surfaceContainerHighest;
 textColor = message.isUserMessage ? colorScheme.onSecondaryContainer : colorScheme.onSurfaceVariant;
 break;
 case MessageType.image:
 case MessageType.text:
 default:
 // Use primary for user, surfaceVariant for AI/default
 bubbleColor = message.isUserMessage ? colorScheme.primaryContainer : colorScheme.surfaceContainerHighest;
 textColor = message.isUserMessage ? colorScheme.onPrimaryContainer : colorScheme.onSurfaceVariant;
 }
 ...
 color: bubbleColor, // Apply calculated bubble color
 borderRadius: BorderRadius.circular(16.0),
 boxShadow: [ BoxShadow( /* ... */ ) ]
 ),
 child: _buildMessageContent(message, textColor), // Pass text color
 ),
 ),
 Padding(
 ...
 // ignore: deprecated_member_use
 icon: Icon(Icons.copy_outlined, size: 16, color: colorScheme.onSurface.withOpacity(0.6)), 
                    tooltip: 'Copy Text',
                    ...
                    onPressed: () {
                       ...
                       // Maybe add context for non-text messages?
                       if (message.type == MessageType.image) textToCopy = "[Image${message.text.isNotEmpty ? ': ${message.text}' : ''}]";
                       if (message.type == MessageType.audio) textToCopy = "[Audio Recording${message.text.isNotEmpty ? ' Transcription: ${message.text}' : ''}]";
           ...
           //   child: Text(
           //     DateFormat('HH:mm').format(message.timestamp), // Requires intl package
  ...
  switch (message.type) {
           ...
           case MessageType.functionResponse:
              ...
              return SelectionArea(
                    child: message.isUserMessage || message.type == MessageType.error || message.type == MessageType.functionResponse
                     ? Text(message.text, style: TextStyle(color: textColor))
                       : MarkdownBody(
                           data: message.text,
                           selectable: false, // Selection handled by SelectionArea parent
              ...
              return Column(
                 ...
                 children: [
                     if (message.imagePath != null)
                       ClipRRect( // Add rounded corners to image
                          ...
                          child: Image.file(
                             ...
                             errorBuilder: (context, error, stackTrace) {
                                return Container(
                                   width: 200, height: 100, color: Colors.grey[300],
                     ...
                     if (message.text.isNotEmpty) ...[
                        const SizedBox(height: 8),
                         SelectionArea(child: Text(message.text, style: TextStyle(color: textColor))),
                     ],
                 ],
              );

            case MessageType.audio:
            Color iconColor = message.isUserMessage ? colorScheme.secondary : colorScheme.primary; // Example icon colors
      Color sliderActiveColor = message.isUserMessage ? colorScheme.secondary : colorScheme.primary;
      Color sliderInactiveColor = sliderActiveColor.withOpacity(0.3);


               final isPlaying = _currentlyPlayingAudioPath == message.audioPath &&
                               (_currentPlayerState == PlayerState.playing || _currentPlayerState == PlayerState.paused);
                final currentPosition = isPlaying ? _audioPosition : Duration.zero;
                final totalDuration = message.audioDuration ?? (isPlaying ? _audioDuration : Duration.zero);
                      ...
                      const SizedBox(width: 8),
                       Expanded(
                         child: Column(
                           ...
                           children: [
                             Text(
                               message.text.startsWith('Error') // Show transcription or placeholder
                                 ? "Audio Recording" // Fallback / Error text
                                  : (message.text.isNotEmpty && message.text != "Audio Recording")
                             ...
                             if (totalDuration > Duration.zero) // Show slider/time only if duration known
                                Row(
                                   children: [
                                      Text(
                                      ...
                                      Expanded(
                                          child: SliderTheme( // Customize slider appearance
                                             data: SliderTheme.of(context).copyWith(
                                                ...
                                                thumbColor: message.isUserMessage ? sliderActiveColor : sliderInactiveColor,
                                                activeTrackColor: message.isUserMessage ? sliderActiveColor : sliderInactiveColor,
                                                inactiveTrackColor: message.isUserMessage ? sliderInactiveColor : sliderActiveColor,
                                              ),
                                             child: Slider(
                                                 ...
                                                 onChanged: (value) async {
                                                    if (_currentlyPlayingAudioPath == message.audioPath) {
            ...
            default:
               return SelectionArea(child: Text(message.text, style: TextStyle(color: textColor)));
       }
 }
 ...
 return Material(
 elevation: 8.0,
 color: theme.cardColor, // Use theme card color for background
 shadowColor: theme.shadowColor, // Use theme shadow color
 child: Padding(
 ...
 icon: Icon(Icons.add_circle_outline, color: colorScheme.primary),
                ...
                onSelected: (value) {
                  switch (value) {
                    case 'camera': _pickImage(ImageSource.camera); break;
                    case 'gallery': _pickImage(ImageSource.gallery); break;
                    // case 'file': _pickGeneralFile(); break; // Uncomment if using file_picker
                  }
                },
                itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                   const PopupMenuItem<String>(value: 'camera', child: ListTile(leading: Icon(Icons.camera_alt), title: Text('Camera'))),
                   const PopupMenuItem<String>(value: 'gallery', child: ListTile(leading: Icon(Icons.photo_library), title: Text('Gallery'))),
                   // const PopupMenuItem<String>(value: 'file', child: ListTile(leading: Icon(Icons.attach_file), title: Text('File'))), // Uncomment if using file_picker
                ],
                 enabled: !_isLoading && !_isRecording && !_isTranscribing, // Disable when busy
             ),
            ...
            Expanded(
              child: TextField(
                ...
                maxLines: 5,
                minLines: 1,
                keyboardType: TextInputType.multiline, // Ensure multiline
                decoration: InputDecoration(
                  hintText: 'Type message...',
                  filled: true,
                  fillColor: colorScheme.surfaceContainerHighest.withOpacity(0.5),
 hintStyle: TextStyle(color: colorScheme.onSurface.withOpacity(0.6)), // Theme hint color
 ...
 borderSide: BorderSide.none,
                ...
                enabled: !_isLoading && !_isRecording && !_isTranscribing, // Disable when busy
                 // onSubmitted: (_) => _showSendButton ? _sendMessageWithData() : null, // Send on submit only if send button shown
              ),
            ),
            const SizedBox(width: 8.0),

            // --- Animated Send / Record Button ---
             AnimatedSwitcher(
                ...
                child: _showSendButton
                  ?
                   // --- Send Button ---
                   IconButton(
                       key: const ValueKey('send_button'), // Key for AnimatedSwitcher
                       icon: const Icon(Icons.send),
                       tooltip: 'Send Message',
                       color: colorScheme.primary,
                       onPressed: (_isLoading || _isInitializing || _isTranscribing) ? null : () => _sendMessageWithData(),
                    )
                  :
                   // --- Record Button ---
                   GestureDetector( // Use GestureDetector for long press behavior
                      key: const ValueKey('record_button'), // Key for AnimatedSwitcher
                      onLongPressStart: (_) => _startRecording(),
                      onLongPressEnd: (_) => _stopRecording(),
                      onLongPressCancel: () => _stopRecording(cancelled: true), // Handle cancellation
                      // Optional: Tap to start/stop if preferred over long press
                      // onTap: () async {
                      //    if (await _inputFileService.isRecording()) {
                      //       _stopRecording();
                      //    } else {
                      ...
                      //    }
                      // },
                      child: AnimatedContainer( // Animate size/color change
                         duration: const Duration(milliseconds: 150),
                         padding: EdgeInsets.all(_isRecording ? 14 : 10), // Grow when recording
                         decoration: BoxDecoration(
 color: _isRecording ? colorScheme.onError : colorScheme.primary,
                             shape: BoxShape.circle,
                         ),
                         child: Icon(
                            Icons.mic,
                            color: _isRecording ? colorScheme.onError : colorScheme.onPrimary,
                            size: _isRecording ? 28 : 24, // Grow icon
                         ),
                      ),
                   ),
             ),
